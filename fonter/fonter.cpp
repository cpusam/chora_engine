...
int main ( int argc, char ** argv )
{
	if (argc < 5)
	{
		printf("Usage: %s font=\"file.ttf\" size=N dim=N color=R/G/B/A text=AbCdE...\n", argv[0]);
	...
	SDL_Surface * bmp = Writer::instance()->render_text_surface(std::string(text),color);
	...
	for (int i = 0, w = 0, h = 0; i < chars.size(); i++)
	{
		...
		{
			h = 0;
			printf("Erro: tamanho dim muito pequeno para tanto caracteres\n");
		}
